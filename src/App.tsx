...
import { Mic, MicOff, Loader2, History, Trash2, Waves, Send, Sparkles, Copy, Check, RotateCcw } from 'lucide-react';
...
function Dashboard() {
  ...
  // Use lazy initialization for state to avoid double render parsing
  const [transcriptions, setTranscriptions] = useState<Transcription[]>(() => {
    const saved = localStorage.getItem('matic_transcriptions');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error("Failed to parse saved transcriptions", e);
      }
    }
    return [];
  });
  ...
  useEffect(() => {
    localStorage.setItem('matic_transcriptions', JSON.stringify(transcriptions));
  }, [transcriptions]);
  ...
  const transcribeAudio = async (blob: Blob) => {
    ...
    try {
      ...
      if (text) {
        const newDoc: Transcription = {
          id: Math.random().toString(36).substring(7),
          text,
          createdAt: new Date().toISOString(),
          status: 'active'
        };
        setTranscriptions(prev => [newDoc, ...prev]);
      }
    } catch (err) {
  ...
  const archiveTranscription = (id: string) => {
    setTranscriptions(prev => prev.map(t => 
      t.id === id ? { ...t, status: 'archived', archivedAt: new Date().toISOString() } : t
    ));
  };

  const restoreTranscription = (id: string) => {
    setTranscriptions(prev => prev.map(t => 
      t.id === id ? { ...t, status: 'active', archivedAt: undefined } : t
    ));
  };
